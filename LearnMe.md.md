...
* [AeroScript Programming Language](#AeroScript-programming-language)
  * [Summary](#summary)
  * [Overview](#overview)
...
AeroScript is a custom language interpreter, written in Go. It has C-style syntax, and is largely inspired by Ruby, Python, Perl and c#
...
This is a sample program using AeroScript language:
...
class RequestMain {
  static fun main() {
    request = new Request()
    request.FirstName = "Yuri Yei ji 123456789"
    request.LastName = "ABC DEFGHI JKL     "
    request.Dept = "Department of Alphabatts"
...
```sh
AeroScript.exe run path/to/file
```
...
AeroScript support two kinds of single line comment and also block comment.
...
AeroScript supports 9 basic data types: `String`, `Int`, `UInt`, `Float`, `Bool`, `Array`, `Hash`, `Tuple` and `Nil`
...
In AeroScript, there are mainly eleven types of constants(Literals).
...
Variables in AeroScript could start with the keyword `let`, or nothing with the
form `variable=value`.
...
Note：Starting from AeroScript 5.0，when the decalared variable already exists, it's value will be overwritten:
...
In AeroScript, you can use enum to define constants.
...
### Meta-Operators
AeroScript has some meta-operators borrowed from perl6.
...
At the moment, AeroScript has six meta-operators：
* <p>~+</p>
* <p>~-</p>
* <p>~*</p>
* <p>~/</p>
...
```
### using statement
In AeroScript, if you have some resources you want to release/free/close, e.g. close opened file, close network connection etc，
...
In AeroScript, you are free to define some operators, but you cannot
...
In AeroScript, integer is treated as an object, so you could call it's methods.
...
In AeroScript, float is also treated as an object, so you could call it's methods.
Please see below examples:
...
In AeroScript, decimal is Arbitrary-precision fixed-point decimal numbers.
...
In AeroScript, you could use [] to initialize an empty array:
...
In AeroScript, there are three types of `string`:
...
In AeroScript, strings are utf8-encoded, you could use utf-8 encoded name as a variable name.
...
### Hash
In AeroScript, the builtin hash will keep the order of keys when they are added to the hash, just like python's orderedDict.
...
In AeroScript, Hash is also an object, so you could use them to operate on hash object:
...
In AeroScript, `tuple` is just like array, but it could not be changed once it has been created.
...
### Extend basic type 
AeroScript also provides support for extending basic types.
...
### Optional type
AeroScript has support for Optional type like java8.

```swift
fun safeDivision?(a, b) {
    if (b == 0){
...
AeroScript has limited support for the oop concept, below is a list of features:
...
The AeroScript parser could parse `public`, `private`, `protected`, but it has no effect in the evaluation phase.
That means AeroScript do not support access modifiers at present.
...
class Animal {
    let name = ""
    fun init(name) {    //'init' is the constructor
        //do somthing
    }
}
```

In AeroScript, all class is inherited from the root class `object`. 
...
AeroScript has support for class `indexer`(like c#). 
...
AeroScript also support class Category like objective-c（C# is called 'extension methods'）.
...
AeroScript also has very simple annotation support like java：
...
AeroScript also include some builtin annotations:
...
Maybe you are curious about why `nil` or `false` have message() function? Because in AeroScript, `nil` and `false`
...
In AeroScript, you could concatenate of different types. See below for examples:
...
AeroScript support list(array,string, range, tuple) comprehensions.
...
AeroScript also support hash comprehension.
...
Function in AeroScript is a first-class object. This means the language supports passing functions as arguments to
...
Before ver5.0, AeroScript do not support multiple return values, But there are many ways to do it.
...
Starting from ver5.0, AeroScript support multiple return values using 'let'.
The returned values are wrapped as a tuple.

```swift
fun testReturn(a, b, c, d=40) {
    return a, b, c, d
}
...
AeroScript has experimental support for working with `go` modules.

If you need to use go`s language package function, you first need to use `RegisterFunctions` or `RegisterVars` to
register `go` language functions or types into AeroScript language.

Below is an example of `main.go`(extracted):

```swift
// Because in AeroScript we already have built in module `fmt`, here we use `fmt` for package name.
...
```
Now, in your AeroScript file, you could use it like below:
...
In AeroScript, there are some standard modules provided for you. e.g. json, sql, sort, fmt, os, logger, time, flag, net, http, etc...
This is a brief introduction of some of the AeroScript standard modules, don't expect it to be thorough.
...
let nameV = flag.string("name", "Seo Yeo YoungYruiYei-ji", "a string")
...
In AeroScript, the `linq` module support seven types of object:
...
Now, AeroScript has a powerful `linq for file` support. it can be used to operate
...
The formatter utility can format the AeroScript language.
The highlighter utility can highlight the AeroScript language to console or html.
